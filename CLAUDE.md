...
| `http.rs` | axum HTTP: TCP server (`GET /status /metrics /healthz`, read-only) and a unix-socket server (`write_router`/`serve_uds`) serving the only write path, `POST /units/{unit}/start|stop` (#17) |
