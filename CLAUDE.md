...
| `classify.rs` | Cmdline → game-claim classification (Steam, patterns) |
...
| `http.rs` | axum HTTP: TCP server (`GET /status /metrics /healthz`, plus the loopback-gated `POST /units/{unit}/start|stop` write routes on Windows only) and a unix-socket server (`write_router`/`serve_uds`) serving those same write routes on Linux (#17) |
...
| `cgroup.rs` | cgroup-based PID → systemd-unit attribution: parses `/proc/<pid>/cgroup` to resolve the owning unit for a GPU process, independent of the process's own name/binary. Feeds `/status` per-unit VRAM (`reconcile.rs`) and per-unit eviction gating (`units.rs`). (Linux-only; stub elsewhere — Windows has no per-process VRAM at all.) |
...
- **Priority ladder & cooperative eviction** — per-unit `priority` (default 50) vs the top-level `game_priority` (default 100); a demand at `P` preempts every unit with `priority < P` (strict, so equal tiers coexist). `busy_cmd` (exit 0 = busy) is what makes a unit a preemption *source* rather than only a target. `yield_cmd`/`resume_cmd` add a cooperative release stage ahead of the stop path, budgeted by per-unit or top-level `yield_timeout_s`. **`yield_cmd` is inert without `busy_cmd`** — release would be unobservable, so the daemon warns, skips the stage, and stops the unit.
- **Detection** — `detect_steam` (on by default), `[[game_patterns]]` (cmdline substrings for non-Steam launchers).
