...
| `units.rs` | Managed-unit lifecycle: stop → poll VRAM → SIGKILL, systemctl or custom `*_cmd` |
| `reconcile.rs` | Reconciliation authority: `/proc` scan → claim set → drive managed units |
| `http.rs` | axum HTTP server: `GET /status /metrics /healthz`, `POST /units/{unit}/start|stop` |
| `procmon.rs` | `cn_proc` netlink listener → debounced reconcile trigger (Linux-only; stub on macOS) |
| `presence.rs` | Physical-input-device watcher via evdev/inotify (Linux-only; stub on macOS) |

**Reconciliation model**: level-triggered, K8s-controller style. `reconcile()` observes ground truth (`/proc` scan, optional GPU procs), recomputes the full claim set, and drives managed units. No delta state — self-heals across crashes and dropped events.
...
# Deployed artifact — static musl binary (matches what CI ships and release.yml publishes)
...
```sh
cargo test                                       # runs on macOS too (pure logic)
cargo fmt --check
cargo clippy --all-targets -- -D warnings
```

CI runs all four in sequence; all must pass on every PR.

## Toolchain

Pinned via `rust-toolchain.toml` to the `stable` channel. Requires Rust ≥ 1.88 (edition 2024, let-chains). Components: `rustfmt`, `clippy`.
...
The annotated example lives at `packaging/config.example.toml`. Every key is optional; the daemon is fully usable with zero config (evicts `ollama.service` on Steam game detection by default).
...
- NVIDIA: `nvidia-smi` on `PATH`; AMD: no extra tooling (reads `/sys/class/drm/`)
- systemd (default); non-systemd hosts use per-unit `*_cmd` overrides

## Conventions

- All pure logic lives in the library crate (`src/lib.rs` re-exports). Binaries only wire things together.
